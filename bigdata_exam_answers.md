...
<table>
<tr><td>📝 <b>الشرح</b></td></tr>
<tr><td>حسب المقرر، تظهر عيوب RDBMS بشكل أوضح عند التعامل مع البيانات الضخمة والبيئات الموزعة.</td></tr>
</table>

**⚠️ العيوب الرئيسية (حسب المقرر)**:

| العيب | المصطلح بالإنجليزية | التفصيل المختصر |
|------|-------------------|----------------|
| التعقيد في البيئات الموزعة والسحابية | Distributed Environments | ليست مناسبة دائمًا عندما تكون البيانات موزعة على عقد متعددة |
| صعوبة إدارة الاتساق والتوفر وتحمل الانقسام | Consistency, Availability, Partition Tolerance | يصعب تحقيقها معًا في الأنظمة الموزعة |
| صعوبة التعامل بكفاءة مع البيانات الضخمة جدًا | Handling Big Data | قد تحدث اختناقات أداء ويستلزم إعادة تصميم |
| تضخم حجم البيانات إلى تيرابايت/بيتابايت | Data Volume (TB/PB) | يمثل تحديًا لنظم RDBMS |
| التوسع الرأسي مكلف | Scale Up Cost | زيادة المعالجات والذاكرة ترفع التكلفة |
| نسبة كبيرة من البيانات شبه/غير منظمة | Semi-/Unstructured Data (~80%) | RDBMS لا يتعامل معها بكفاءة |

> **💡 مثال واقعي**: الاتجاه إلى NoSQL مثل Cassandra أو MongoDB لمعالجة الحجم الكبير والبيئات الموزعة.

📖 **المصدر**: `Lec 1 Lect 2 Lec 3.pdf` صفحات 80-81 (ومقدمة عيوب البيئات الموزعة قبلها)
